package com.android.javacard.jcserver;
...
public class JavaCardHostApp {
  ...
  public void establishConnectionToSimulator() throws IOException {
    ...
    mCadClient = CadDevice.getCadClientInstance(CadDevice.PROTOCOL_T1,
  ...
  public void exchangeTheAPDUWithSimulator()
    ...
    // Exchange the APDUs
    System.out.println("data in length "
            + Utils.byteArrayToHexString(mApdu.dataIn) + " Lc = " + mApdu.Lc);
  ...
  public void setTheDataIn(byte[] data) {
    if (data.length != mApdu.Lc) {
      System.err.println(
              "The number of data in the array are more than expected");
    } else {
      // set the data to be sent to the applets
      mApdu.dataIn = data;
      /*
       * for (int dataIndx = 0; dataIndx < data.length; dataIndx++) {
       * System.out.println("dataIn" + dataIndx + ": " +
       * atrToHex(data[dataIndx])); }
       */
  ...
  public boolean decodeApduBytes(byte[] apduBytes) {
    ...
    if (apduBytes.length > 5) {
      ...
      if (apduBytes[4] == 0) {
        ...
        setTheDataIn(Arrays.copyOfRange(apduBytes, 5,
                5 + Byte.toUnsignedInt(apduBytes[4])));
      }
    }
